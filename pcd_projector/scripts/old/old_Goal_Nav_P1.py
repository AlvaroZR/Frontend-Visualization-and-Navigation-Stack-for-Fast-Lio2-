...
def main():
    ...
    with open(GOALS_FILE, 'r') as file:
        ...
        header = next(reader)
        goals = [(int(row[0]), float(row[1]), float(row[2])) for row in reader]

    rate = rospy.Rate(10)
    goal_index = 0

    while not rospy.is_shutdown() and goal_index < len(goals):
